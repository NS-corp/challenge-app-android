...
public class AuthActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    private void authUserTask(int userId){

        App.getServerApi().authUserRequest(userId).enqueue(new ResponseCallback<AuthUserResponseData>() {
            @Override
            public void onRequestSuccess(AuthUserResponseData responseData) {
                UserInfo userInfo = responseData.getUserInfo();
                if(userInfo == null){
                    Log.e("responseData", "UserInfo is null");
                    return;
                }
